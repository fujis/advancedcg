...
void ElasticPBD::projectBendingConstraint(float ks)
{
	...
	for(int i = 0; i < m_iNumEdge; i++){
		...
		float c23 = glm::length(glm::cross(p2, p3));
		float c24 = glm::length(glm::cross(p2, p4));
		if(c23 < 1.0e-6 || c24 < 1.0e-6) continue;
		glm::vec3 q3 = (glm::cross(p2, n2)+glm::cross(n1, p2)*d)/c23;
		glm::vec3 q4 = (glm::cross(p2, n1)+glm::cross(n2, p2)*d)/c24;
		glm::vec3 q2 = -(glm::cross(p3, n2)+glm::cross(n1, p3)*d)/c23 - (glm::cross(p4, n1)+glm::cross(n2, p4)*d)/c24;
